...
| 层级 | 技术选型 | 说明 | 实现状态 |
|------|----------|------|----------|
| UI 框架 | SwiftUI | 声明式 UI，跨设备适配 | **已实现** |
| 数据持久化 | SwiftData | Swift 原生 ORM，替代 Core Data | **已实现** |
| 云同步 | CloudKit (via SwiftData) | 自动镜像到 iCloud | **已实现** |
| OCR 引擎 | Vision Framework | 离线文字识别，保护隐私 | **已实现** |
| 语音识别 | Speech Framework | 离线语音转文字 | **已实现** |
| 图表 | Swift Charts | Apple 原生图表框架 | **已实现** |
| 本地认证 | LocalAuthentication | FaceID/TouchID | **已实现** |
| 日历 | 自研农历算法 | 农历与公历互转 | **已实现** |
| 触觉反馈 | UIFeedbackGenerator | Taptic Engine | **已实现** |
| 数据导出 | CSV 导出 | 导出记录、联系人、统计等 | **已实现** |
| 内购 | StoreKit 2 | 高级版买断 | **已实现** |
| 通讯录 | Contacts Framework | 联系人关联 | 规划中 |
| 定位 | Core Location | 获取城市信息用于礼俗建议 | 规划中 |
| 小组件 | WidgetKit | 桌面小组件 | 规划中 |
| Siri 集成 | AppIntents | Siri 快捷指令 | 规划中 |
| Excel 导出 | CoreXLSX (SPM) | 生成 .xlsx 文件 | 规划中（当前仅支持 CSV） |
| PDF 生成 | PDFKit | 电子借条 PDF | 规划中 |
| Dynamic Island | ActivityKit | 婚礼现场极速模式 | 规划中 |
...
> **状态：规划中** — 当前版本联系人由用户手动创建管理，尚未集成系统通讯录。
...
> **当前实现状态：** CSV 导出已完成（支持记录、联系人、统计、提醒事件的导出，以及按账本/时间范围筛选）。Excel (.xlsx)、长图导出、PDF 借条为规划中功能。

#### Excel 导出 (CoreXLSX) — 规划中
...
> **状态：本章节描述的功能（WidgetKit、AppIntents/Siri、Dynamic Island）均为规划中，尚未实现。**

### 9.1 桌面小组件 (WidgetKit) — 规划中
...
### 9.3 Dynamic Island (ActivityKit) — 规划中
...
> **当前状态：** 项目目前无任何第三方依赖，完全基于 Apple 原生框架。CoreXLSX 为规划中的依赖（当前数据导出使用纯 CSV 格式）。

| 依赖 | 用途 | 引入方式 | 版本 | 状态 |
|------|------|----------|------|------|
| [CoreXLSX](https://github.com/CoreOffice/CoreXLSX) | Excel .xlsx 文件生成与读取 | SPM | 最新稳定版 | 规划中 |

> **设计原则**：尽可能使用 Apple 原生框架，最小化第三方依赖，降低维护成本和审核风险。

### SPM 配置（规划中）

待引入 CoreXLSX 时，在 Xcode 中通过 `File → Add Package Dependencies` 添加：
...
### Phase 1：核心基础 (Day 1-15) — 已完成

| 任务 | 优先级 | 预估工时 | 状态 |
|------|--------|----------|------|
| 数据模型定义 (SwiftData) | P0 | 2天 | **已完成** |
| ModelContainer + CloudKit 配置 | P0 | 1天 | **已完成** |
| 账本 CRUD (Repository + ViewModel) | P0 | 2天 | **已完成** |
| 记录录入（手动模式） | P0 | 3天 | **已完成** |
| 联系人管理 | P0 | 2天 | **已完成** |
| 主 Tab 导航框架 | P0 | 1天 | **已完成** |
| 首页仪表盘 | P1 | 2天 | **已完成** |
| 基础 UI 组件库 | P0 | 2天 | **已完成** |

### Phase 2：智能功能 (Day 16-30) — 大部分完成

| 任务 | 优先级 | 预估工时 | 状态 |
|------|--------|----------|------|
| OCR 识别模块 | P0 | 4天 | **已完成** |
| 语音记账模块 | P1 | 3天 | **已完成** |
| 统计分析（Swift Charts） | P0 | 3天 | **已完成** |
| 通讯录同步 | P1 | 2天 | 规划中 |
| 农历计算服务 | P1 | 2天 | **已完成** |
| 本地通知（生日/节日提醒） | P1 | 1天 | **已完成** |

### Phase 3：增值功能 (Day 31-45) — 部分完成

| 任务 | 优先级 | 预估工时 | 状态 |
|------|--------|----------|------|
| 礼俗知识库建设 | P1 | 3天 | 规划中 |
| 数据导出 (CSV) | P0 | 3天 | **已完成**（CSV 格式） |
| 数据导出 (Excel/PDF) | P1 | 3天 | 规划中 |
| 电子借条模板 | P2 | 2天 | 规划中 |
| 生物识别锁 | P0 | 1天 | **已完成** |
| 本地快照备份 | P1 | 2天 | 规划中 |
| 年度人情报告 | P2 | 3天 | 规划中 |
| 内购（高级版） | P0 | 2天 | **已完成**（StoreKit 2） |

### Phase 4：系统集成 & 上线 (Day 46-60) — 待开始

| 任务 | 优先级 | 预估工时 | 状态 |
|------|--------|----------|------|
| 桌面 Widget 开发 | P1 | 3天 | 规划中 |
| Siri 快捷指令集成 | P1 | 2天 | 规划中 |
| 深色模式完整适配 | P0 | 2天 | 规划中 |
| iPad 大屏适配 | P1 | 2天 | 规划中 |
| 性能优化 & 内存调优 | P0 | 2天 | 规划中 |
| TestFlight 内测 | P0 | 3天 | 规划中 |
| App Store 提交审核 | P0 | 1天 | 规划中 |
